...
- **Nie abstrahuj na siłę security primitives** — `JwtRoleConverter`, `AuthenticatedUser`, `@CurrentUser`, `CurrentUserArgumentResolver`, `SecurityConfig`, a także `GlobalExceptionHandler` + `ErrorResponse` (uniform error contract) są **świadomie zduplikowane** między `orchestrator` a `file-service` (2 serwisy × ~80 linii; wspólny `common-*` to premature abstraction przy 2 konsumentach). Zmieniasz jeden — zsynchronizuj drugi.

    ...
    def _load_samples(self, protocols_file, data_dir, extra_fake_dir):
        ...
        if self.is_train and extra_fake_dir and os.path.exists(extra_fake_dir):
            fake_files = glob.glob(
                os.path.join(extra_fake_dir, "**", "*.wav"), recursive=True
        ...
        if self.is_train and self.fake_to_real_ratio > 0:
            ...
            if len(fake_samples) > max_fake:
                ...
                print(
                    f"  ⚖️  Downsampling FAKE: {len(fake_samples)} (z ratio {self.fake_to_real_ratio}:1)"
                )
        samples = real_samples + fake_samples
        if self.is_train:
            random.shuffle(samples)
        print(
            f"  📊 Dataset: {len(real_samples)} REAL + {len(fake_samples)} FAKE = {len(samples)} total"
        )
        return samples
    ...
    def setup(self, stage=None):
        if stage == "fit" or stage is None:
            ...
            self.val_dataset = ASVspoofDataset(
                protocols_file=os.path.join(
                    self.data_root, "ASVspoof2019_LA_cm_protocols", "ASVspoof2019.LA.cm.dev.trl.txt"
                ),

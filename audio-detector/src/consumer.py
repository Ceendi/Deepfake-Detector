...
PROCESSED_AUDIO_TOTAL = Counter(
    "audio_processed_total", "Total number of audio files processed", ["status"]
)
...
def run_consumer(health_state: dict) -> None:
    while True:
        try:
            ...
            ch.queue_bind(
                queue="analysis.results", exchange=EXCHANGE, routing_key="analysis.results"
            )
            ch.queue_bind(
                queue="analysis.progress", exchange=EXCHANGE, routing_key="analysis.progress"
            )
